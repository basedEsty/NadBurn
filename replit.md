...
### Bundled token-logo seed (auto-refreshed)

`artifacts/ash/src/lib/seed-logos.json` ships a small, pre-computed
subset of the top tokens per chain (CoinGecko market-cap rank +
per-platform list top-up) so common rows render with the right logo
on first paint, before the runtime Uniswap + CoinGecko lists finish
loading. Generated by `scripts/src/build-seed-logos.ts`
(`pnpm --filter @workspace/scripts run build-seed-logos`).

`.github/workflows/refresh-seed-logos.yml` re-runs the generator
every Monday 06:00 UTC (and on `workflow_dispatch`) and opens a PR
with the diff via `peter-evans/create-pull-request`. The script
refuses to overwrite the checked-in file when CoinGecko returns a
clearly degraded result (zero entries, or any chain shrinks by >30%),
so a partial CoinGecko outage turns the workflow run red instead of
silently committing a worse seed. Set `REFRESH_FORCE=1` (or check the
"force" box on a manual dispatch) to override the guard for a real
ranking shift.
